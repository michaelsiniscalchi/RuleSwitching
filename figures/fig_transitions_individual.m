...
for i = 1:numel(ruleName)
    ...
    for j = 1:numel(blocks)
        ax(j) = nexttile; 
        X = T.diffSimilarity(blocks(j)).trialIdx;
        Y = T.diffSimilarity(blocks(j)).trials;
...
% %% Sound-to-Action Transitions
% fig(2) = figure('Name',[T.sessionID '(Action)' T.params.stat]);
% nRows = ceil(numel(actionIdx)/3);
% fig(2).Position = [0,50,1850,300*nRows];
% tiledlayout(nRows,3);
% blocks = actionIdx;
% for j = 1:numel(blocks)
%     ax(j) = nexttile;
%     X = T.diffSimilarity(blocks(j)).trialIdx;
%     Y = T.diffSimilarity(blocks(j)).trials;
%     plot(X,Y,'Color',params.Color{2},'Marker','o','LineStyle','none'); hold on;
%     ylims(j,:) = ylim;
%     
% end
% % Standardize YLims
