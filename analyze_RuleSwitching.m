...
if do_plot.validation_ITIs || do_plot.validation_ROIs || do_plot.validation_alignment
    %Setup figure directories
    save_dir = fullfile(dirs.figures,'Validation Checks');   %Figures directory
    create_dirs(save_dir); %Create dir for these figures
    
    if do_plot.validation_ITIs
    ...
    if do_plot.validation_ROIs %***WIP***
    ...
    if do_plot.validation_alignment 
        % Validate alignment of fluorescence timeseries
...
%
% 200128 try to plot all bar graphs as beeswarms (except perhaps the preference plots - those could be compact boxplots)