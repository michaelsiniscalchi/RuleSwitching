...
if calculate.fluorescence
    ...
    for i = 1:numel(expData)
        ...
        if calculate.transitions
            ...
            transitions = calc_transitionResults(img_beh,params.transitions);
...
% DEVO Plots: transition analyses
if do_plot.transitions
    
    for i = 1:numel(expData)
        %Load data
        load(mat_file.results(i),'transitions');
        save_dir = fullfile(dirs.figures,'Neural transitions');   %Figures directory
        create_dirs(save_dir); %Create dir for these figures
        figs(i) = fig_transitions_cell_x_trial( transitions, params.figs.transitions ); %Mean dF/F as f(trial)
        %figs(i+numel(expData)) = fig_transitions_heatmap( transitions, params.figs.transitions );
    end
    save_multiplePlots(figs,save_dir); %save as FIG and PNG
    clearvars figs;
end
...
if do_plot.summary_behavior
    ...
    for i=1:numel(cellType)
        figs(i) = fig_summary_behavior(stats.behavior,cellType{i},params.figs.behavior);
    end
    save_multiplePlots(figs,save_dir); %save as FIG and PNG
    clearvars figs;
end
